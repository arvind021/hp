...
def generate_signal_ml(symbol):
    ...
    # Model load nahi hai — disk se try karo pehle
    if symbol not in models or models[symbol][0] is None:
        m, sc, ac = load_model_local(symbol)
        if m:
            models[symbol] = (m, sc, ac)
            print(f"  📂 {symbol} model loaded from disk")
        else:
            # Disk pe nahi hai — train karo
    ...
    if model is None:
        print(f"  ⚠️ {symbol}: No model available")
        return None, 0, ""
